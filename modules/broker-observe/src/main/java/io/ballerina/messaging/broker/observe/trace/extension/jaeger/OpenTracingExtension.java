 ...
 * under the License.
 */
package io.ballerina.messaging.broker.observe.trace.extension.jaeger;

import com.uber.jaeger.Configuration;
import io.ballerina.messaging.broker.observe.trace.InvalidConfigurationException;
import io.ballerina.messaging.broker.observe.trace.OpenTracer;
...
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.DEFAULT_REPORTER_FLUSH_INTERVAL;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.DEFAULT_REPORTER_HOSTNAME;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.DEFAULT_REPORTER_LOG_SPANS;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.DEFAULT_REPORTER_MAX_BUFFER_SPANS;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.DEFAULT_REPORTER_PORT;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.DEFAULT_SAMPLER_PARAM;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.DEFAULT_SAMPLER_TYPE;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.REPORTER_FLUSH_INTERVAL_MS_CONFIG;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.REPORTER_HOST_NAME_CONFIG;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.REPORTER_LOG_SPANS_CONFIG;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.REPORTER_MAX_BUFFER_SPANS_CONFIG;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.REPORTER_PORT_CONFIG;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.SAMPLER_PARAM_CONFIG;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.SAMPLER_TYPE_CONFIG;
import static io.ballerina.messaging.broker.observe.trace.extension.jaeger.Constants.TRACER_NAME;
...
public class OpenTracingExtension implements OpenTracer {
    ...
    public Tracer getTracer(String tracerName, Properties configProperties, String serviceName)
        ...
        if (!tracerName.equalsIgnoreCase(TRACER_NAME)) {
            throw new InvalidConfigurationException("Unexpected tracer name! " +
                                                    "The tracer name supported by this extension is : " + TRACER_NAME +
                                                    " but found : " + tracerName);
        }
    ...
    private void validateConfiguration(Properties configuration) {
        ...
        setValidatedIntegerConfig(configuration, SAMPLER_PARAM_CONFIG, DEFAULT_SAMPLER_PARAM);
        ...
        if (configValue == null) {
            ...
            try {
                ...
                logger.warn(String.format("Open tracing configuration for tracer name - %s expects " +
                                          "configuration element : %swith integer type but found non integer : %s ! " +
                                          "Therefore assigning default value : %d for %s configuration.", TRACER_NAME,
                                          configName, configValue.toString(), defaultValue, configName));
                configuration.put(configName, defaultValue);
            }
        }
    }
    ...
    private void setValidatedBooleanConfig(Properties configuration, String configName, boolean defaultValue) {
        ...
        if (configValue == null) {
            ...
            if (configStringValue.equalsIgnoreCase(Boolean.TRUE.toString())
                ...
                logger.warn(String.format("Open tracing configuration for tracer name - %s expects " +
                                          "configuration element : %swith boolean type (true/false) but found non " +
                                          "boolean : %s ! Therefore assigning default value : %s for %s configuration.",
